...
public static class GoEmitter
{
    public static void Emit(GenerationModel gm, string outDir, BuildReport report)
    {
        ...
        foreach (var op in gm.Operations)
        {
            ...
            var guards = GuardsFile(op, report);
    ...
    // FINDING merkezi: Go'da `dynamic` yok → predicate yapısal-ama-derlenen emit EDİLEMEZ.
    static string? GuardsFile(GmOperation op, BuildReport report)
    {
        var funcs = new List<string>();
        void Add(string kind, int i, string text)
        {
            report.Unsupported(kind, $"{op.Id}#{kind}{i}", "Go: dynamic predicate ctx yok — typed-context kararı gerek (GM seam bulgusu)");
            funcs.Add($"// {kind} (yapısal): {text}\nfunc {char.ToLowerInvariant(op.Id[0]) + op.Id[1..]}{kind}{i}(req {ReqName(op)}) (bool, error) {{\n\treturn false, nil // ponytail: stub; typed predicate stratejisi GM kararı\n}}");
        }
        for (var i = 0; i < op.Op.Validation.Count; i++) Add("Validation", i, op.Op.Validation[i].Text);
        for (var i = 0; i < op.Op.Rule.Count; i++) Add("Rule", i, op.Op.Rule[i].Text);
